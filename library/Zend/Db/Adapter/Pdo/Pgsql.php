    ...
    public function describeTable($tableName, $schemaName = null)
    {
        ...
        foreach ($result as $key => $row) {
            ...
            if ($row[$type] == 'varchar' || $row[$type] == 'bpchar' ) {
                if (preg_match('/character(?: varying)?(?:\((\d+)\))?/', isset($row[$complete_type]) ? $row[$complete_type] : "", $matches)) {
                    if (isset($matches[1])) {
                ...
                if (preg_match("/^'(.*?)'::(?:character varying|bpchar)$/", isset($defaultValue) ? $defaultValue : "", $matches)) {
            ...
            if ($row[$contype] == 'p') {
                ...
                $identity = (bool) (preg_match('/^nextval/', isset($row[$default_value]) ? $row[$default_value] : ""));
            }
